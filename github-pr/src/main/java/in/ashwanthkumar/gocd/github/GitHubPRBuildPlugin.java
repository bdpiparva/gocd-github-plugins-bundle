...
import in.ashwanthkumar.gocd.github.provider.GitHubProvider;
...
import in.ashwanthkumar.gocd.github.validator.FieldValidator;
...
public class GitHubPRBuildPlugin implements GoPlugin {
    ...
    static final String REQUEST_VALIDATE_SCM_CONFIGURATION = "validate-scm-configuration";
    private static final String REQUEST_CHECK_SCM_CONNECTION = "check-scm-connection";
    private static final String REQUEST_PLUGIN_CONFIGURATION = "go.plugin-settings.get-configuration";
    private static final String REQUEST_PLUGIN_VIEW = "go.plugin-settings.get-view";
    private static final String REQUEST_VALIDATE_PLUGIN_CONFIGURATION = "go.plugin-settings.validate-configuration";
    ...
    private static final String BRANCH_TO_REVISION_MAP = "BRANCH_TO_REVISION_MAP";
    ...
    GoPluginApiResponse handleGetLatestRevision(GoPluginApiRequest goPluginApiRequest) {
        ...
        try {
            ...
            Map<String, Object> response = new HashMap<String, Object>();
            String defaultBranch = (isEmpty(gitConfig.getBranch())) ? "master" : gitConfig.getBranch();
    ...
    private String removeUsernameAndPassword(String message, GitConfig gitConfig) {
        ...
        if (isNotBlank(gitConfig.getPassword())) {
            messageForDisplay = message.replaceAll(gitConfig.getPassword(), "****");
        }
        if (isNotBlank(gitConfig.getUsername())) {
            messageForDisplay = messageForDisplay.replaceAll(gitConfig.getUsername(), "****");
        }
    ...
    GitConfig getGitConfig(Map<String, String> configuration) {
        GitConfig gitConfig = new GitConfig(
                ...
                trimToNull(configuration.get("defaultBranch")),
                true,
    ...
    private void validateUrl(GitConfig gitConfig, Map<String, Object> fieldMap) {
    ...
    private void checkConnection(GitConfig gitConfig, Map<String, Object> response, List<String> messages) {
        if (StringUtil.isEmpty(gitConfig.getUrl())) {
