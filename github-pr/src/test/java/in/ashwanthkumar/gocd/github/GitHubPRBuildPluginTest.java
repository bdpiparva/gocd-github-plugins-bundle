...
import in.ashwanthkumar.gocd.github.provider.GHUtils;
...
import static in.ashwanthkumar.gocd.github.GitHubPRBuildPlugin.REQUEST_VALIDATE_SCM_CONFIGURATION;
...
public class GitHubPRBuildPluginTest {
    private static final String TEST_DIR = "/tmp/" + UUID.randomUUID();
    private static File propertyFile;
    private static boolean propertyFileExisted = false;
    ...
    public void shouldHandleInvalidURLCorrectly_ValidationRequest() {
        ...
        GoPluginApiResponse response = new GitHubPRBuildPlugin().handle(createGoPluginApiRequest(REQUEST_VALIDATE_SCM_CONFIGURATION, request));
    ...
    public void shouldBuildWhitelistedBranch() {
        ...
        when(gitFolderFactory.create(anyString())).thenReturn(mock(File.class));
        GitHubPRBuildPlugin plugin = new GitHubPRBuildPlugin(
                new GitHubProvider(),
                gitFactory,
                gitFolderFactory,
    ...
    private void verifyValidationSuccess(String url) {
        ...
        GitHubPRBuildPlugin plugin = new GitHubPRBuildPlugin();
        plugin.setProvider(new GitHubProvider());
        GoPluginApiResponse response = plugin.handle(createGoPluginApiRequest(REQUEST_VALIDATE_SCM_CONFIGURATION, request));
